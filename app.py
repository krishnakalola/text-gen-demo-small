
import streamlit as st
import pandas as pd
import os

##############
#
#  Global Constants
#

# misc global constants
max_gen_len = 100
max_tuple_len = 4
max_chars = 200
command_phrase = 'cmd'

# global data about sentence content
allowed_words = 'a the ' \
			  + 'child dog cat giraffe box bike ' \
			  + 'saw played ran sat ' \
			  + 'with after from on'
allowed_words = [ word.lower() for word in allowed_words.split( ' ' ) ]
start_token = ' '
end_token = '.'

# get training data from disk if there is any
text_file = 'all-inputs.txt'
with st.spinner( 'Loading...' ):
	if os.path.isfile( text_file ):
		with open( text_file, 'r' ) as f:
			sentences = [ line[:-1] for line in f.readlines() ]
	else:
		sentences = [ ]


##############
#
#  Utility Functions
#

# add a sentence to the training data or report why the sentence is invalid
def add_training_data ( sentence ):
	if len( sentence ) > max_chars:
		st.error( f'Your sentence was more than {max_chars} characters.' )
		return
	sentence = sentence.replace( ',', '' ).replace( '.', '' ).replace( '!', '' )
	words = [ word.lower() for word in sentence.split( ' ' ) ]
	errors = [ word for word in words if word not in allowed_words ]
	if len( words ) == 0 or len( words ) == 1 and words[0] == '':
		st.error( 'Your example sentence contains no words.' )
		return
	if len( errors ) > 0:
		st.error( 'Not an allowed word: ' + errors[0] )
		return
	with open( text_file, 'a' ) as f:
		f.write( ' '.join( words ) + '\n' )
	st.success( 'Saved sentence to training data' )

# obey an administrative command (e.g., clear out the training data)
def obey_command ( command ):
	if command == 'clear':
		os.remove( text_file )
		st.success( 'Training data file removed.' )
	else:
		st.error( 'Did not understand that command' )

# function that fetches tuples from the training data, of arbitrary size
def tuple_table ( num_prev_words=1 ):
	rows = [ ]
	for sentence in sentences:
		tokens = [ start_token ] * num_prev_words \
			   + sentence.split( ' ' ) \
			   + [ end_token ]
		for index in range( num_prev_words, len( tokens ) ):
			rows.append( tokens[index-num_prev_words:index+1] )
	colnames = [ 'Next', 'Previous' ]
	for count in range( 2, num_prev_words+1 ):
		colnames.append( f'{count} words ago' )
	colnames.reverse()
	return pd.DataFrame( rows, columns = colnames )

# create a frequency table from the set of tuples in the training data
def tall_frequency_table ( num_prev_words=1 ):
	base = tuple_table( num_prev_words )
	base['Frequency'] = 1
	base = base.groupby( base.columns[:-1].tolist() )[['Frequency']].agg( 'count' )
	return base

# in the case where num_prev_words==1, we can represent the table in wide form, too
def wide_frequency_table ():
	result = base_frequency_table().pivot_table(
		index='Previous', columns='Next',
		values='Frequency', aggfunc='sum' ).fillna( 0 )
	if len( result ) == 0:
		return result
	# move START row to the top
	result = pd.concat( [
		result.loc[[start_token],:],
		result.drop(start_token, axis=0)
	], axis=0 )
	# move END row to the left
	columns = [ col for col in result.columns if col is not end_token ]
	result = result[[ end_token, *columns ]]
	# clean up and be done
	result.index.name = None
	return result

# generate a random sentence from the given table of tuples
def generate_sentence ( tuple_table ):
	n = len( tuple_table.columns ) - 1
	sentence = [ start_token ] * n
	while sentence[-1] is not end_token and len( sentence ) < max_gen_len:
		subset = df
		for i in range( 0, n ):
			subset = subset[subset.iloc[:,i].isin( [ sentence[-n+i] ] )]
		sentence.append( subset['Next'].sample( 1 ).iloc[0] )
	sentence = ' '.join( sentence[n:-1] )
	return sentence[0].upper() + sentence[1:] + '.'


##############
#
#  User Interface
#

# title
st.title( 'Very Small Language Model' )

# structure of the app as a set of tabs
input_tab, text_tab, matrix_tab, generator_tab = st.tabs( [
	'Add training data',
	'View training data',
	'Learned frequencies',
	'Text generation'
] )

# show tab where users can add a sentence to the training data
with input_tab:
	st.write( 'To submit a new sentence to help train this "AI," '
			+ 'type it in the box below and click the Submit button.  '
			+ f'Sentences may be no more than {max_chars} characters.  '
			+ 'You may use ONLY these words in your sentence:' )
	st.write( '**' + ( ", ".join( allowed_words ) ) + '**' )
	sentence = st.text_input(
		'Your example sentence',
		placeholder='A child saw the dog.'
	)
	if st.button( 'Submit example sentence' ):
		if sentence[:len(command_phrase)+1] == command_phrase+' ':
			obey_command( sentence[len(command_phrase)+1:] )
		else:
			add_training_data( sentence )

# show tab that lets users see the current training data
with text_tab:
	if len( sentences ) > 0:
		st.write( '\n'.join( [
		    ' 1. ' + sentence[0].upper() + sentence[1:] + '.'
			for sentence in sentences
		] ) )
	else:
		st.write( '(No training data entered yet.)' )

# show tab that lets users see the frequency tables computed from the training data
with matrix_tab:
	if len( sentences ) == 0:
		st.warning( 'No training data available yet.  Add some first.' )
	else:
		n = st.number_input( 'Number of words to use for prediction',
							 1, max_tuple_len, 1, key='matrix_tab_n' )
		st.write( '(Scroll down to see full table.)' )
		df = tall_frequency_table( n )
		st.dataframe(
			df,
			use_container_width=True,
			column_config={
				'Frequency' : st.column_config.ProgressColumn(
					max_value=df['Frequency'].max(), format='%d' )
			}
		)
		# st.write( wide_frequency_table() )

# show tab that generates random new sentences based on what was learned
with generator_tab:
	if len( sentences ) == 0:
		st.warning( 'No training data available yet.  Add some first.' )
	else:
		n = st.number_input( 'Number of words to use for prediction',
							 1, max_tuple_len, 1, key='generator_tab_n' )
		if st.button( 'Generate some sentences' ):
			df = tuple_table( n )
			st.success( '\n'.join( [
				f' 1. {generate_sentence(df)}' for _ in range(10)
			] ) )

